...
const InitialCreditSummary = () => {
  ...
  const status = "Creditworthiness Assessment";
  const { data, isError, isLoading, error } = useQuery({
    queryKey: ["Summary-of-Application-stage4", applicationId, status],
  ...
  const stage4 = data.percentage?.stage4;

  const points = {
    borrowingCap: `${stage4.borrowing_loan_capacity || 0} UAH`,
    requestedLoan: `${stage4.requested_loan || 0} UAH`,
    perLoan: `${
      (stage4.percentage_borrowing_capacity &&
        Number(stage4.percentage_borrowing_capacity).toFixed(2)) ||
      0
    }%`,
  };
  ...
  return (
    <>
      ...
      {data?.application?.remarksStage4 && (
